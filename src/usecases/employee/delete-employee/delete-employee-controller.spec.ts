...
import { DeleteEmployeeSpy, throwError } from '@test-mocks'
...
type SutTypes = {
  ...
  service: DeleteEmployeeSpy
}
const makeSut = (): SutTypes => {
  const service = new DeleteEmployeeSpy()
  const sut = new DeleteEmployeeController(service)
  return {
    sut,
    service,
  }
...
describe('Test Suite for delete-employee-controller.spec', () => {
  ...
  test('Ensure that controller to return http response 404 when id is not found', async () => {
    ...
    vi.spyOn(service, 'delete').mockImplementationOnce(() =>
      throwError(new DeleteEmployeeNotFoundError('Error')),
    )
    const data = { id: '1' }
    const result = await sut.handle(data)
