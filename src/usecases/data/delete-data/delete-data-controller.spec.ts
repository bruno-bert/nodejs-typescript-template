...
import { DeleteDataSpy, throwError } from '@test-mocks'
...
type SutTypes = {
  sut: DeleteDataController
  service: DeleteDataSpy
}
const makeSut = (): SutTypes => {
  const service = new DeleteDataSpy()
  const sut = new DeleteDataController(service)
  return {
    sut,
    service,
  }
...
describe('Test Suite for delete-data-controller.spec', () => {
  ...
  test('Ensure that controller to return http response 404 when id is not found', async () => {
    ...
    vi.spyOn(service, 'delete').mockImplementationOnce(() =>
      throwError(new DeleteDataNotFoundError('Error')),
    )
    const data = { id: '1' }
    const result = await sut.handle(data)
